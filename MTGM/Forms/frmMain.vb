...
'       VgBar.ShowInTaskbar = False
    ...
    Dim VpCardData() As String
        ...
        While Not VpTournois.EndOfStream
            VpLine = VpTournois.ReadLine
            VpCardData = VpLine.Split("#")
            Try
                ...
                ElseIf VpCardData.Length = 10 Then
                    VgDBCommand.CommandText = "Insert Into Autorisations (Title, T1, T1r, T15, M, Pioneer, T2, Bloc, Blocoff, 1V1, Multi, MTGO, MTGOoff) Values ('" + VpCardData(0).Replace("'", "''") + "', " + (Not VpCardData(1).EndsWith("no")).ToString + ", " + (VpCardData(1).EndsWith("r")).ToString + ", " + (Not VpCardData(2).EndsWith("no")).ToString + ", " + (Not VpCardData(3).EndsWith("no")).ToString + ", " + (Not VpCardData(4).EndsWith("no")).ToString + ", " + (Not VpCardData(5).EndsWith("no")).ToString + ", " + (Not VpCardData(6).EndsWith("no")).ToString + ", " + (VpCardData(6).EndsWith("dk")).ToString + ", " + (Not VpCardData(7).EndsWith("no")).ToString + ", " + (Not VpCardData(8).EndsWith("no")).ToString + ", " + (Not VpCardData(9).EndsWith("no")).ToString + ", " + (VpCardData(9).EndsWith("dk")).ToString + ");"
                    VgDBCommand.ExecuteNonQuery
                ElseIf VpCardData.Length = 11 And Not VpLine.Contains("alchemy") Then
                    VgDBCommand.CommandText = "Insert Into Autorisations (Title, T1, T1r, T15, M, Pioneer, Historic, T2, Bloc, Blocoff, 1V1, Multi, MTGO, MTGOoff) Values ('" + VpCardData(0).Replace("'", "''") + "', " + (Not VpCardData(1).EndsWith("no")).ToString + ", " + (VpCardData(1).EndsWith("r")).ToString + ", " + (Not VpCardData(2).EndsWith("no")).ToString + ", " + (Not VpCardData(3).EndsWith("no")).ToString + ", " + (Not VpCardData(4).EndsWith("no")).ToString + ", " + (Not VpCardData(5).EndsWith("no")).ToString + ", " + (Not VpCardData(6).EndsWith("no")).ToString + ", " + (Not VpCardData(7).EndsWith("no")).ToString + ", " + (VpCardData(7).EndsWith("dk")).ToString + ", " + (Not VpCardData(8).EndsWith("no")).ToString + ", " + (Not VpCardData(9).EndsWith("no")).ToString + ", " + (Not VpCardData(10).EndsWith("no")).ToString + ", " + (VpCardData(10).EndsWith("dk")).ToString + ");"
                    VgDBCommand.ExecuteNonQuery
                ElseIf VpCardData.Length = 11 Then
                    VgDBCommand.CommandText = "Insert Into Autorisations (Title, T1, T1r, T15, M, Pioneer, Explorer, T2, 1V1, Multi, Historic, Alchemy) Values ('" + VpCardData(0).Replace("'", "''") + "', " + (Not VpCardData(1).EndsWith("no")).ToString + ", " + (VpCardData(1).EndsWith("r")).ToString + ", " + (Not VpCardData(2).EndsWith("no")).ToString + ", " + (Not VpCardData(3).EndsWith("no")).ToString + ", " + (Not VpCardData(4).EndsWith("no")).ToString + ", " + (Not VpCardData(5).EndsWith("no")).ToString + ", " + (Not VpCardData(6).EndsWith("no")).ToString + ", " + (Not VpCardData(7).EndsWith("no")).ToString + ", " + (Not VpCardData(8).EndsWith("no")).ToString + ", " + (Not VpCardData(9).EndsWith("no")).ToString + ", " + (Not VpCardData(10).EndsWith("no")).ToString + ");"
                    VgDBCommand.ExecuteNonQuery
                End If
            Catch
...
'           VgBar.ShowInTaskbar = False
    ...
    Dim VpStrs() As String
        ...
        If Me.dlgOpen4.FileName <> "" Then
            VpFile = New StreamReader(Me.dlgOpen4.FileName)
            Do While Not VpFile.EndOfStream
                VpStr = VpFile.ReadLine.Replace(","""","""",""""", "")
                If VpStr.Contains(""",") Then
                    VpStrSub = VpStr.Substring(VpStr.LastIndexOf(""",") + 2)
                    VpStrs = VpStrSub.Split(",")
                    If VpStrs.Length = 9 AndAlso VpStrs(2) <> "0" Then
                        VgDBCommand.CommandText = "Update Card Set UrzaId = " + VpStrs(1) + " Where MultiverseId = " + VpStrs(2) + ";"
                        VgDBCommand.ExecuteNonQuery
                    ElseIf VpStrs(2) = "0" Then
                        VpStrSub = VpStr.Replace(", ", "¤").Replace("0,000", "0k").Replace("""", "")
                        VpStrs = VpStrSub.Split(",")
                        If VpStrs.Length = 25 Then
                            VgDBCommand.CommandText = "Update Card Inner Join Series On Card.Series = Series.SeriesCD Set Card.UrzaId = " + VpStrs(17) + " Where Card.Title = '" + VpStrs(0).Replace("¤", ", ").Replace("0k", "0,000").Replace("'", "''") + "' And Series.SeriesNM_UG = '" + VpStrs(15).Replace(":", "").Replace("¤", ", ").Replace("0k", "0,000").Replace("'", "''") + "';"
                            VgDBCommand.ExecuteNonQuery
                        End If
                    End If
                End If
...
'       VgBar.ShowInTaskbar = True
    ...
    '-----------------------------------------------------------------------
        If VpCard = "" Or Me.IsMainReaderBusy Then
            ...
            Me.picAutM.Image = Me.imglstAutorisations.Images.Item(9)
            Me.picAutT2.Image = Me.imglstAutorisations.Images.Item(12)
            Me.picAutP.Image = Me.imglstAutorisations.Images.Item(15)
            Me.picAutE.Image = Me.imglstAutorisations.Images.Item(18)
            Me.picAut1V1.Image = Me.imglstAutorisations.Images.Item(21)
            Me.picAutMulti.Image = Me.imglstAutorisations.Images.Item(24)
            Me.picAutH.Image = Me.imglstAutorisations.Images.Item(27)
            Me.picAutA.Image = Me.imglstAutorisations.Images.Item(30)
        Else
            VgDBCommand.CommandText = "Select T1r, T1, T15, M, T2, Pioneer, Explorer, Commanderoff, [1V1], Multi, Arenaoff, Historic, Alchemy From Autorisations Where Title = '" + VpCard.Replace("'", "''") + "';"
            VgDBReader = VgDBCommand.ExecuteReader
            With VgDBReader
                If .Read Then
                    'Autorisations T1 (Vintage)
                    If .GetBoolean(0) Then
                        Me.picAutT1.Image = Me.imglstAutorisations.Images.Item(0)   'Restriction à 1 item
                    ElseIf .GetBoolean(1) Then
                    ...
                    If .GetBoolean(2) Then
                        Me.picAutT15.Image = Me.imglstAutorisations.Images.Item(4)
                    Else
                    ...
                    If .GetBoolean(5) Then
                        Me.picAutP.Image = Me.imglstAutorisations.Images.Item(13)
                    Else
                        Me.picAutP.Image = Me.imglstAutorisations.Images.Item(14)
                    End If
                    'Autorisations Explorer
                    If .GetBoolean(6) Then
                        Me.picAutE.Image = Me.imglstAutorisations.Images.Item(16)
                    Else
                        Me.picAutE.Image = Me.imglstAutorisations.Images.Item(17)
                    End If
                    'Autorisations 1 vs 1
                    If .GetBoolean(8) Then
                        Me.picAut1V1.Image = Me.imglstAutorisations.Images.Item(19)
                    Else
                        Me.picAut1V1.Image = Me.imglstAutorisations.Images.Item(20)
                    End If
                    'Autorisations Multi
                    If .GetBoolean(9) Then
                        Me.picAutMulti.Image = Me.imglstAutorisations.Images.Item(22)
                    Else
                        Me.picAutMulti.Image = Me.imglstAutorisations.Images.Item(23)
                    End If
                    ...
                    End If
                    'Autorisations Alchemy
                    If .GetBoolean(12) Then
                        Me.picAutA.Image = Me.imglstAutorisations.Images.Item(28)
                    Else
                        Me.picAutA.Image = Me.imglstAutorisations.Images.Item(29)
                    End If
                Else
                    Call Me.LoadAutorisations("")
                End If
