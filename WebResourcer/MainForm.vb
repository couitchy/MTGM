...
Public Partial Class MainForm
    ...
    Dim VpJSONInfos As clsFullInfos = Nothing
        ...
        'Vérification des noms des cartes
        Call Me.CheckMultiNames(VpJSONInfos)
        'Infos sur l'édition
    ...
    End Sub
    Private Sub CheckMultiNames(VpJSONInfos As clsFullInfos)
        For Each VpCard As clsFullInfos.clsFullDataInfos.clsFullCardInfos In VpJSONInfos.data.cards
            If VpCard.layout = "transform" AndAlso VpCard.name.Contains(" // ") Then
                VpCard.names = New List(Of String)
                VpCard.names.AddRange(VpCard.name.Split(New String() {" // "}, StringSplitOptions.None))
                VpCard.name = VpCard.faceName
            End If
        Next VpCard
    End Sub
